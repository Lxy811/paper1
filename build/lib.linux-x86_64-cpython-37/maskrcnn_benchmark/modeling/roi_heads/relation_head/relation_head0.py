...
from .L21_loss import L2ClearBatchLoss, TripletLoss
...
class ROIRelationHead(torch.nn.Module):
    ...
    ####################################################
        ...
        output_losses = node_losses(refine_logits, obj_labels)  # predicted node labels (objects)

        e_loss, edges_fg, edges_bg = edge_losses(relation_logits,           # predicted edge labels (predicates)
                                           rel_labels,   # ground truth edge labels (predicates)
                                           self.cfg.LOSS,
                                           return_idx=True,
                                           loss_weights=(self.cfg.ALPHA, self.cfg.BETA, self.cfg.GAMMA))
        
        add_losses.update(e_loss)
        output_losses.update(add_losses)
    #################################################
