    ...
    def forward(self, inputs, initial_state=None, labels=None, boxes_for_nms=None):
        ...
        for i, l_batch in enumerate(batch_lengths):
            ...
            if self.training:
                ...
                # if is_bg.dim() > 0:
                if is_bg.shape[0] > 0:
                    # print(type(labels_to_embed), type(nonzero_pred), type(is_bg), is_bg.shape, is_bg.dim())
                    # print(labels_to_embed, nonzero_pred, is_bg)
                    labels_to_embed[is_bg.squeeze(1)] = nonzero_pred[is_bg.squeeze(1)]
                out_commitments.append(labels_to_embed)
                # print(type(labels_to_embed), labels_to_embed, type(labels_to_embed+1), labels_to_embed+1)
