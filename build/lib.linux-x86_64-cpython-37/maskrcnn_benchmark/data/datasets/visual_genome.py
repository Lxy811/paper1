...
class VGDataset(torch.utils.data.Dataset):
    ...
    def get_statistics(self):
        fg_matrix, bg_matrix, alpha_mat = get_VG_statistics(
            img_dir=self.img_dir, roidb_file=self.roidb_file, dict_file=self.dict_file,
            image_file=self.image_file, filter_empty_rels=self.filter_empty_rels,
            num_val_im=self.num_val_im, zs_file=self.zs_file,
            filter_duplicate_rels=self.filter_duplicate_rels,
            filter_non_overlap=self.filter_non_overlap, must_overlap=True
        )
        eps = 1e-3
        bg_matrix += 1
    ...
    def get_groundtruth(self, index, evaluation=False, flip_img=False):
        ...
        """add by lin xin"""
        fn = self.filenames[index].split('/')[-1].split('.')[-2]
        fn = int(fn)
        # ipdb.set_trace()
        if hasattr(target, 'image_fn'):
            target.image_fn = fn
        ...
        """ add by lin xin"""
        """
        pairs_count = ((relation_map + relation_map.t()) > 0).float().sum(0)
        pairs_count = pairs_count / pairs_count.sum()
        gamma = torch.clamp(-((1 - 2.0*pairs_count) ** 5.0) * torch.log(2.0*pairs_count), max=2.0)

        target.add_field("gamma", gamma)
        """
        # att_map = torch.from_numpy(self.att_maps[index]).to(box)
        # assert att_map.shape[0] == num_box
        # target.add_field("attention_map", att_map, is_triplet=True)
...
def get_VG_statistics(img_dir, roidb_file, dict_file, image_file, filter_empty_rels, num_val_im, zs_file,
                      filter_duplicate_rels, filter_non_overlap, must_overlap=True):
    train_data = VGDataset(split='train', img_dir=img_dir, roidb_file=roidb_file, dict_file=dict_file,
                           image_file=image_file, filter_empty_rels=filter_empty_rels, num_val_im=num_val_im,
                           zs_file=zs_file, filter_duplicate_rels=filter_duplicate_rels,
    ...
    for ex_ind in tqdm(range(len(train_data))):
        ...
        for (o1, o2), gtr in zip(o1o2, gt_relations[:,2]):
            ...
            o1, o2, gtr = int(o1), int(o2), int(gtr)
            tpt_list.append((o1, o2, gtr))
        # For the background, get all of the things that overlap.
        o1o2_total = gt_classes[np.array(
            box_filter(gt_boxes, must_overlap=must_overlap), dtype=int)]
        for (o1, o2) in o1o2_total:
            bg_matrix[o1, o2] += 1
    alpha_mat = re_weight(tpt_list)
    return fg_matrix, bg_matrix, alpha_mat


def get_VG_statistics_new(img_dir, roidb_file, dict_file, image_file, must_overlap=True):
    """add by lin xin"""
    train_data = VGDataset(split='train', img_dir=img_dir, roidb_file=roidb_file, 
                        dict_file=dict_file, image_file=image_file, num_val_im=5000, 
                        filter_duplicate_rels=False)
...
def load_graphs(roidb_file, split, num_im, num_val_im, filter_empty_rels, filter_non_overlap, zs_file=None):
    """
    ...
    for i in range(len(image_index)):
        ...
        if zs_tpt is not None:
            new_rels = []
            for item_rel in rels:
                tpt = (gt_classes_i[item_rel[0]], gt_classes_i[item_rel[1]], item_rel[2])
                if tpt in zs_tpt:
                    continue
                new_rels.append(item_rel)
            if len(new_rels) == 0:
