...
class LxyPredictor1(nn.Module):
    def __init__(self, config, in_channels):
        ...
        self.cfg = config  # 保存配置
        self.attribute_on = config.MODEL.ATTRIBUTE_ON  # 是否启用属性预测
        # load parameters
        self.num_obj_cls = config.MODEL.ROI_BOX_HEAD.NUM_CLASSES  # 物体类别数
        self.num_att_cls = config.MODEL.ROI_ATTRIBUTE_HEAD.NUM_ATTRIBUTES  # 属性类别数
        self.num_rel_cls = config.MODEL.ROI_RELATION_HEAD.NUM_CLASSES  # 关系类别数

        # 根据配置设置模式
        if self.cfg.MODEL.ROI_RELATION_HEAD.USE_GT_BOX:
            if self.cfg.MODEL.ROI_RELATION_HEAD.USE_GT_OBJECT_LABEL:
                self.mode = 'predcls'  # 预测分类模式
            else:
                self.mode = 'sgcls'  # 场景图分类模式
        else:
            self.mode = 'sgdet'  # 场景图检测模式

        assert in_channels is not None
        num_inputs = in_channels  # 输入通道数
        self.use_vision = config.MODEL.ROI_RELATION_HEAD.PREDICT_USE_VISION  # 是否使用视觉特征
        self.use_bias = config.MODEL.ROI_RELATION_HEAD.PREDICT_USE_BIAS  # 是否使用偏置
        self.prune_rate = config.MODEL.ROI_RELATION_HEAD.PRUNE_RATE  # 剪枝率
        self.lambda_ = config.MODEL.ROI_RELATION_HEAD.LAMBDA_  # 损失权重系数

        # load class dict
        statistics = get_dataset_statistics(config)  # 获取数据集统计信息
        obj_classes, rel_classes, att_classes = (
            statistics["obj_classes"],  # 物体类别
            statistics["rel_classes"],  # 关系类别
            statistics["att_classes"],  # 属性类别
        )
        ...
        self.alpha_mat = alpha_mat.cuda()  # 将 alpha 矩阵移到 GPU
        self.zs_indicator = torch.zeros(self.alpha_mat.shape, device=self.alpha_mat.device, dtype=torch.long)  # 初始化零样本指示器
        self.zs_indicator[self.alpha_mat == 0] = 1  # 标记零样本
        self.zs_indicator[self.alpha_mat != 0] = -1  # 标记非零样本

        path = './prune_ckpt/prune_mat.pkl'
        self.prune_mat = torch.load(path).to(self.alpha_mat.device)  # 加载语义约束向量矩阵
        values = self.prune_mat[self.prune_mat != 0]  # 获取非零值

        sort_values = torch.sort(values)[0]  # 对非零值排序
        length = values.shape[0]  # 非零值总数
        r_length = int((length - 1) * self.prune_rate)  # 计算语义约束向量阈值索引
        if r_length >= len(sort_values):
            self.prune_rate_v = sort_values[-1] + 1  # 设置语义约束向量阈值
        else:
            self.prune_rate_v = sort_values[r_length]  # 设置语义约束向量阈值

        self.test_indicator = copy.deepcopy(self.zs_indicator)  # 复制零样本指示器用于测试
        self.test_indicator[(self.alpha_mat == 0) & (self.prune_mat < self.prune_rate_v)] = -1  # 更新测试指示器
        self.zs_indicator[(self.alpha_mat == 0) & (self.prune_mat < self.prune_rate_v)] = 0  # 更新零样本指示器

        self.none_prune_idx = torch.zeros(self.zs_indicator.shape, device=self.zs_indicator.device)  # 初始化非剪枝索引
        self.none_prune_idx[(self.alpha_mat == 0) & (self.prune_mat >= self.prune_rate_v)] = 1  # 标记非剪枝区域，保存合理的三元组
        assert self.num_obj_cls == len(obj_classes)  # 验证物体类别数
        assert self.num_att_cls == len(att_classes)  # 验证属性类别数
        assert self.num_rel_cls == len(rel_classes)  # 验证关系类别数
        # module construct
        ...
        if config.GLOBAL_SETTING.BASIC_ENCODER == 'Self-Attention':
            self.context_layer = TransformerContext(config, obj_classes, rel_classes, in_channels)  # 自注意力上下文层
        elif config.GLOBAL_SETTING.BASIC_ENCODER == 'Cross-Attention':
            self.context_layer = CA_Context(config, obj_classes, rel_classes, in_channels)  # 交叉注意力上下文层
        elif config.GLOBAL_SETTING.BASIC_ENCODER == 'Hybrid-Attention':
            self.context_layer = SHA_Context(config, obj_classes, rel_classes, in_channels)  # 混合注意力上下文层

        # post decoding
        self.hidden_dim = config.MODEL.ROI_RELATION_HEAD.CONTEXT_HIDDEN_DIM  # 隐藏层维度
        self.pooling_dim = config.MODEL.ROI_RELATION_HEAD.CONTEXT_POOLING_DIM  # 池化维度
        self.post_emb = nn.Linear(self.hidden_dim, self.hidden_dim * 2)  # 后处理嵌入层
        self.post_cat = nn.Linear(self.hidden_dim * 2, self.pooling_dim)  # 后处理拼接层

        self.rel_compress = nn.Linear(self.pooling_dim, self.num_rel_cls)  # 关系压缩层
        self.ctx_compress = nn.Linear(self.hidden_dim*2, self.num_rel_cls)  # 上下文压缩层

        layer_init(self.rel_compress, xavier=True)  # 初始化关系压缩层
        layer_init(self.ctx_compress, xavier=True)  # 初始化上下文压缩层

        # initialize layer parameters
        layer_init(self.post_emb, 10.0 * (1.0 / self.hidden_dim) ** 0.5, normal=True)  # 初始化后处理嵌入层
        layer_init(self.post_cat, xavier=True)  # 初始化后处理拼接层

        # 处理维度不匹配
        if self.pooling_dim != config.MODEL.ROI_BOX_HEAD.MLP_HEAD_DIM:
            self.union_single_not_match = True  # 标记维度不匹配
            self.up_dim = nn.Linear(config.MODEL.ROI_BOX_HEAD.MLP_HEAD_DIM, self.pooling_dim)  # 维度提升层
            layer_init(self.up_dim, xavier=True)  # 初始化维度提升层
        else:
            self.union_single_not_match = False  # 标记维度匹配

        ################# get model configs############
        self.Knowledge_Transfer_Mode = config.GLOBAL_SETTING.GCL_SETTING.KNOWLEDGE_TRANSFER_MODE  # 知识迁移模式
        self.no_relation_restrain = config.GLOBAL_SETTING.GCL_SETTING.NO_RELATION_RESTRAIN  # 是否无关系约束
        self.zero_label_padding_mode = config.GLOBAL_SETTING.GCL_SETTING.ZERO_LABEL_PADDING_MODE  # 零标签填充模式
        self.knowledge_loss_coefficient = config.GLOBAL_SETTING.GCL_SETTING.KNOWLEDGE_LOSS_COEFFICIENT  # 知识损失系数
        # generate the auxiliary lists
        self.group_split_mode = config.GLOBAL_SETTING.GCL_SETTING.GROUP_SPLIT_MODE  # 分组模式
        num_of_group_element_list, predicate_stage_count = get_group_splits(config.GLOBAL_SETTING.DATASET_CHOICE, self.group_split_mode)  # 获取分组信息
        self.max_group_element_number_list = generate_num_stage_vector(num_of_group_element_list)  # 生成最大分组元素列表
        self.incre_idx_list, self.max_elemnt_list, self.group_matrix, self.kd_matrix = get_current_predicate_idx(
            num_of_group_element_list, 0.1, config.GLOBAL_SETTING.DATASET_CHOICE)  # 获取当前谓词索引
        self.sample_rate_matrix = generate_sample_rate_vector(config.GLOBAL_SETTING.DATASET_CHOICE, self.max_group_element_number_list)  # 生成采样率矩阵
        self.bias_for_group_split = generate_current_sequence_for_bias(num_of_group_element_list, config.GLOBAL_SETTING.DATASET_CHOICE)  # 生成分组偏置

        self.num_groups = len(self.max_elemnt_list)  # 分组数
        self.rel_compress_all, self.ctx_compress_all = self.generate_muti_networks(self.num_groups)  # 生成多网络
        self.CE_loss = nn.CrossEntropyLoss()  # 交叉熵损失

        # 处理偏置
        if self.use_bias:
            self.freq_bias_all = self.generate_multi_bias(config, statistics, self.num_groups)  # 生成多偏置

        # 知识迁移相关
        if self.Knowledge_Transfer_Mode != 'None':
            self.NLL_Loss = nn.NLLLoss()  # NLL 损失
            self.pre_group_matrix = torch.tensor(self.group_matrix, dtype=torch.int64).cuda()  # 分组矩阵
            self.pre_kd_matrix = torch.tensor(self.kd_matrix, dtype=torch.float16).cuda()  # 知识蒸馏矩阵
            self.criterion_loss = nn.CrossEntropyLoss()  # 交叉熵损失
        ############################################################

    def gen_none_prune_idx(self, pair_preds):
        pred = torch.cat(pair_preds, dim=0)  # 拼接预测对
        none_prune_idx = self.none_prune_idx[pred[:, 0], pred[:, 1], :]  # 获取非剪枝索引，标记需要保留的关系
        zs_test_cal = self.test_indicator[pred[:, 0], pred[:, 1], :]  # 获取测试指示器
        return none_prune_idx, zs_test_cal  # 返回非剪枝索引和测试指示器

    def self_weight_calibrate(self, pair_preds):
        pred = torch.cat(pair_preds, dim=0)  # 拼接预测对
        alpha_v = -self.alpha_mat[pred[:, 0], pred[:, 1], :]  # 计算 alpha 值
        alpha_v[alpha_v == 0] = 1  # 将零值设为 1
        return alpha_v  # 返回校准后的 alpha 值
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        # 前向传播   
        # 获取物体概率分数(N, num_obj_cls)，其中 N 是物体数量，num_obj_cls 是物体类别总数、
        # 物体预测，(N,)，其中 N 是物体数量，每个元素是一个类别索引
        # 003--混合注意力关系特征构建模块
        # 边缘上下文 捕捉了物体对之间的语义和空间关系，为后续关系分类（预测关系类别）提供关键特征
        obj_dists, obj_preds, edge_ctx = self.context_layer(roi_features, proposals, logger)  
        # post decode
        edge_rep = self.post_emb(edge_ctx)  # 目标解码器--嵌入边缘上下文
        edge_rep = edge_rep.view(edge_rep.size(0), 2, self.hidden_dim)  # 重塑边缘表示
        head_rep = edge_rep[:, 0].contiguous().view(-1, self.hidden_dim)  # 获取头部表示
        tail_rep = edge_rep[:, 1].contiguous().view(-1, self.hidden_dim)   # 获取尾部表示 #形状同edge_ctx.shape【189，512】O【2672，4096】R

        num_rels = [r.shape[0] for r in rel_pair_idxs]  # 关系数量
        num_objs = [len(b) for b in proposals]     # 假设num_objs = [4, 5, 3]  # 物体数量
        assert len(num_rels) == len(num_objs)  # 验证关系和物体数量一致

        head_reps = head_rep.split(num_objs, dim=0)  # 分割头部表示 #那么第一个张量的形状为 (4, 512)，第二个张量的形状为 (5, 512)。。。。。
        tail_reps = tail_rep.split(num_objs, dim=0)  # 分割尾部表示
        obj_preds = obj_preds.split(num_objs, dim=0)  # 分割物体预测

        # from object level feature to pairwise relation level feature
        prod_reps = []
        pair_preds = []
        for pair_idx, head_rep, tail_rep, obj_pred in zip(rel_pair_idxs, head_reps, tail_reps, obj_preds):
            prod_reps.append(torch.cat((head_rep[pair_idx[:,0]], tail_rep[pair_idx[:,1]]), dim=-1))  # 拼接头部和尾部表示 #head_rep[idx[0]] 和 tail_rep[idx[1]] 分别表示从 head_rep 和 tail_rep 中选择对应索引位置的特征向量。然后，使用 torch.cat() 函数将这两个特征向量沿着特征维度（dim=-1）进行拼接，得到 concatenated_rep
            pair_preds.append(torch.stack((obj_pred[pair_idx[:,0]], obj_pred[pair_idx[:,1]]), dim=1))  # 拼接预测对
        prod_rep = cat(prod_reps, dim=0)  # 拼接所有表示
        pair_pred = cat(pair_preds, dim=0)  # 拼接所有预测对

        ctx_gate = self.post_cat(prod_rep)  # 后处理拼接
#######################################################################

        # 使用视觉特征
        if self.use_vision:
            if self.union_single_not_match:
                visual_rep = ctx_gate * self.up_dim(union_features)  # 维度提升后乘以上下文门
            else:
                visual_rep = ctx_gate * union_features  # 直接乘以上下文门

        # print(visual_rep.shape) #torch.Size([3126, 4096])  # 打印视觉表示形状
        # print(edge_ctx.shape) #torch.Size([242, 512])  # 打印边缘上下文形状
        # print(pair_preds.shape)  # 打印预测对形状

        # print(prod_rep.shape)  #torch.Size([3410, 1024]) 3410是检测到的对象？ # 打印拼接表示形状
        # print(rel_dists.shape) #torch.Size([3410, 51])  150种object和50种predicate # 打印关系分布形状
        none_prune_idx, zs_test_cal = self.gen_none_prune_idx(pair_preds)  # 获取非剪枝索引和测试指示器
        eps = 1e-5  # 防止除零

        if self.training:
            add_losses = {}  # 附加损失
            pair_gt_preds = []
            rel_compress_test = self.rel_compress_all[-1]  # 获取最后一个关系压缩层
            ctx_compress_test = self.ctx_compress_all[-1]  # 获取最后一个上下文压缩层
            rel_dists = rel_compress_test(visual_rep) + ctx_compress_test(prod_rep)  # 计算关系分布
            obj_labels = [proposal.get_field("labels") for proposal in proposals]  # 获取物体标签
            for pair_idx, obj_lbl in zip(rel_pair_idxs, obj_labels):
                pair_gt_preds.append(torch.stack((obj_lbl[pair_idx[:, 0]], obj_lbl[pair_idx[:, 1]]), dim=1))  # 拼接真实预测对

            bias_mcal = self.self_weight_calibrate(pair_preds)  # 校准权重
            bias_mce = self.self_seen_weight_calibrate(pair_preds)  # 校准已见权重
            # print(bias_mcal.shape) #torch.Size([3410, 51])  torch.Size([3126, 51])  # 打印校准权重形状
            # print(bias_mce.shape) #torch.Size([3410, 51]) torch.Size([3126, 51])  # 打印已见权重形状
            bias_rel_dists = rel_dists + bias_mcal  # 应用校准权重
            # print(bias_rel_dists.shape) #torch.Size([3410, 51]) torch.Size([3126, 51])  # 打印校准后关系分布形状

            bias_rel_prob = torch.softmax(bias_rel_dists, dim=-1)  # 计算 softmax 概率
            # 计算未见类概率和
            sum_unseen = torch.sum(bias_rel_prob * (none_prune_idx == 1), dim=-1)  

            if torch.mean(sum_unseen) < eps:
                loss_cal = -torch.log(torch.mean(sum_unseen) + eps)  # 计算校准损失（防止除零）
            else:
                loss_cal = -torch.log(torch.mean(sum_unseen))  # 计算校准损失
            #图空间节点约束
            add_losses["loss_cal"] = self.lambda_ * loss_cal  # 添加校准损失
            rel_dists = rel_dists + bias_mce  # 应用已见权重

            ce_criterion = nn.CrossEntropyLoss()  # 交叉熵损失
            rel_lbl = cat(rel_labels, dim=0)  # 拼接关系标签

            non_zero_mask = rel_lbl != 0  # 非零标签掩码
            zero_mask = rel_lbl == 0  # 零标签掩码
            rel_fbg_loss = []
            if rel_lbl[zero_mask].shape[0] != 0:
                bg_rel_loss = ce_criterion(rel_dists[zero_mask], rel_lbl[zero_mask].long())  # 背景关系损失
                rel_fbg_loss.append(3 * bg_rel_loss)  # 加权背景损失
            if rel_lbl[non_zero_mask].shape[0] != 0:
                fg_rel_loss = ce_criterion(rel_dists[non_zero_mask], rel_lbl[non_zero_mask].long())  # 前景关系损失
                rel_fbg_loss.append(fg_rel_loss)  # 添加前景损失
            rel_fbg_loss = sum(rel_fbg_loss) / 4  # 细粒度平均损失
            add_losses["rel_ce_loss"] = rel_fbg_loss  # 图细粒度节点约束
            rel_dists = to_onehot(rel_lbl, self.num_rel_cls)  # 转换为 one-hot 编码

            ###########################################
            rel_labels = cat(rel_labels, dim=0)  # 拼接关系标签
            max_label = max(rel_labels)  # 最大标签值

            num_groups = self.incre_idx_list[max_label.item()]  # 获取分组数
            if num_groups == 0:
                num_groups = max(self.incre_idx_list)  # 使用最大分组数
            cur_chosen_matrix = []

            for i in range(num_groups):
                cur_chosen_matrix.append([])  # 初始化分组矩阵

            for i in range(len(rel_labels)):
                rel_tar = rel_labels[i].item()  # 获取关系目标
                if rel_tar == 0:
                    if self.zero_label_padding_mode == 'rand_insert':
                        random_idx = random.randint(0, num_groups - 1)  # 随机插入
                        cur_chosen_matrix[random_idx].append(i)  # 添加到随机分组
                        ...
                        if self.zero_label_padding_mode == 'rand_choose':
                            rand_zeros = random.random()  # 随机选择
                        else:
                            rand_zeros = 1.0  # 全部包含
                        if rand_zeros >= 0.4:
                            for zix in range(len(cur_chosen_matrix)):
                                cur_chosen_matrix[zix].append(i)  # 添加到所有分组
                else:
                    rel_idx = self.incre_idx_list[rel_tar]  # 获取关系索引
                    random_num = random.random()  # 随机数
                    for j in range(num_groups):
                        act_idx = num_groups - j
                        threshold_cur = self.sample_rate_matrix[act_idx - 1][rel_tar]  # 获取采样阈值
                        if random_num <= threshold_cur or act_idx < rel_idx:
                            # print('%d-%d-%d-%.2f-%.2f'%(i, rel_idx, act_idx, random_num, threshold_cur))  # 打印分组信息
                            for k in range(act_idx):
                                cur_chosen_matrix[k].append(i)  # 添加到分组
                            break
            ...
            ctx_compress_test = self.ctx_compress_all[-1]  # 获取最后一个上下文压缩层
            rel_dists = rel_compress_test(visual_rep) + ctx_compress_test(prod_rep)  # 计算关系分布
            rel_dists = rel_dists + 2*zs_test_cal  # 应用测试指示器
            if self.use_bias:
                rel_bias_test = self.freq_bias_all[-1]  # 获取最后一个偏置
                rel_dists = rel_dists + rel_bias_test.index_with_labels(pair_pred.long())  # 应用偏置
            add_losses = {}  # 空附加损失
        obj_dists = obj_dists.split(num_objs, dim=0)  # 分割物体分布
        rel_dists = rel_dists.split(num_rels, dim=0)  # 分割关系分布
        return obj_dists, rel_dists, add_losses  # 返回物体分布、关系分布和附加损失
