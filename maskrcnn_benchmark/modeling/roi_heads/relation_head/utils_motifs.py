...
#对于单词列表中的每个单词，代码会尝试从预训练的词向量中获取相应的词向量，并将其存储在张量中-获取语义信息L
def obj_edge_vectors(names, wv_dir, wv_type='glove.6B', wv_dim=300):
    # 加载词向量模型，返回词典、词向量数组和词向量维度
    wv_dict, wv_arr, wv_size = load_word_vectors(wv_dir, wv_type, wv_dim)

    # 创建一个形状为(len(names), wv_dim)的张量，用于存储词向量，初始化为正态分布
...
def rel_edge_vectors(names, wv_dir, wv_type='glove.6B', wv_dim=300):
    wv_dict, wv_arr, wv_size = load_word_vectors(wv_dir, wv_type, wv_dim)
    ...
    for i, token in enumerate(names):
        ...
        if wv_index is not None:
            ...
            wv_index = []
            for x in wv_index:
                if wv_dict.get(x, None) is not None:
                    wv_index.append(wv_dict.get(x, None))
            if len(wv_index) > 0:
                vectors[i] = sum([wv_arr[x] for x in wv_index]) / len(wv_index)
                # vectors[i] = sum([wv_arr[x] for x in wv_index])
            else:
                print("fail on {}".format(token))
    return vectors
